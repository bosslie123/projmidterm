...
class AuthController extends Controller
{
    ...
    public function register(Request $request) {
        ...
        Mail::send('verification-email', ['user'=>$user], function($mail) use ($user) {
            ...
            $mail->from('bosslie1234@gmail.com', 'IPT Systems emailVerify');
        });

        Nexmo::message()->send([
            'to' => '639551343565',
            'from' => '639483298381',
            'text' => 'You have been successfully created an account!'
        ]);
    ...
    public function verification(User $user, $token) {
        if($user->remember_token!==$token) {
            return redirected('/login')->with('Error', 'The token is invalid.');
        }
